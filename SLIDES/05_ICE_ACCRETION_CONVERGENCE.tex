...
\begin{itemize}
    \item At L1, the median predicted ice mass is approximately $2.79$ kg, with an IQR of $0.20$ kg ($7.2\%$), indicating moderate code-to-code variability.
...
\begin{block}{\scriptsize Assessment}
\begin{itemize}
    \item Surface roughness affects the predicted ice geometry, particularly the horn height, extent, and orientation.
    \item<2-> The sensitivity to roughness is participant-dependent, with some submissions exhibiting larger geometric changes than others.
    \item<2-> Nevertheless, substantial differences remain between participants even at comparable roughness levels, indicating that roughness alone does not explain the observed code-to-code variability.
\end{itemize}
\end{block}
...
\begin{block}{\scriptsize Assessment}
\begin{itemize}
    \item The upper-horn angle exhibits substantial participant-to-participant variability at the finest grid level.
    \item Grid refinement generally produces smaller changes than the overall code-to-code spread, although the grid sensitivity varies between submissions.
    \item Refinement therefore does not lead to a common horn-angle prediction across participants.
\end{itemize}
\end{block}
...
\begin{block}{\scriptsize Assessment}
\begin{itemize}
    \item The upper-horn angle is largely insensitive to droplet-distribution resolution for most submissions.
...
\begin{block}{\scriptsize Assessment}
\begin{itemize}
    \item Surface roughness affects the predicted upper-horn angle at the mid-span slice.
    \item<2-> The roughness response is participant-dependent, with some submissions showing stronger sensitivity than others.
    \item<2-> Differences remain between participants even at comparable roughness levels, indicating that roughness alone does not explain the horn-angle spread.
\end{itemize}
\end{block}
