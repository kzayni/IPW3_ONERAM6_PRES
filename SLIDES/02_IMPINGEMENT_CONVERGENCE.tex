...
\begin{frame}{\small ONERA M6 -- Collection Efficiency (15 Bins, L1)}
\scriptsize
...
\only<1>{
\centering
\includegraphics[width=0.5\textwidth]{FIGURES/IMPINGEMENT/tc_oneram6_L1_beta_bins15_vs_s_slice_0p75_roughness_1mm.png}
...
\begin{itemize}
    \item Good code-to-code agreement is observed in the collection-efficiency distribution at the middle spanwise station ($y=0.75$ m).
...
{\centering
\includegraphics[width=0.495\textwidth]{FIGURES/IMPINGEMENT/tc_oneram6_water_mass_vs_n_1mm_bins15_all_grid_levels.png}
\hfill
\includegraphics[width=0.495\textwidth]{FIGURES/IMPINGEMENT/tc_oneram6_water_mass_vs_n_1mm_bins15_all_grid_levels_relative_to_l1.png}
\par}
...
\begin{block}{\scriptsize Observations}
\begin{itemize}
    \item Despite the limited dataset, variability remains low on L1/L2 ($\sim1\%$ IQR) and increases on the coarser L3/L4 grids.
    \item No clear dependence on droplet diameter is observed over the relatively narrow $6.16$--$33.84~\mu$m range.
    \item Grid resolution appears to have a stronger influence on variability than droplet size.
\end{itemize}
\end{block}
...
\begin{block}{\scriptsize Observations}
\begin{itemize}
    \item Due to near-peak $\beta$ oscillations, impingement width is retained as the primary profile-based metric, defined using the $\beta=0.001$ threshold.
    \item Most submissions show limited sensitivity between L1 and L2.
    \item Larger, non-monotonic deviations appear on coarser grids, indicating increased sensitivity of the impingement limits to grid resolution.
\end{itemize}
\end{block}
...
\begin{block}{\scriptsize Observations}
\begin{itemize}
    \item Impingement width systematically increases from 1 to 15 bins, with noticeable differences remaining between 7 and 15 bins.
    \item In contrast, integrated water mass varies by only approximately $2\%$ for most submissions.
    \item Droplet-distribution resolution therefore affects impingement extent more strongly than total collected water mass.
\end{itemize}
\end{block}
...
\begin{itemize}
    ...
    \item Impingement limits are more sensitive to spatial resolution.
\end{itemize}
\end{block}
